...
def _check_deps() -> tuple[bool, str]:
    missing = []
    for pkg in ["pypdfium2", "pypdf", "reportlab"]:
        try:
            __import__(pkg)
        except ImportError:
            missing.append(pkg)
...
from src.pipeline import E2APipeline, PipelineConfig  # noqa: E402
...
if st.button("Translate to Arabic", type="primary", use_container_width=True):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        output_path = Path(tmpdir) / (
            Path(uploaded_file.name).stem + "_ar.pdf"
        )
        ...
        if report.success and output_path.exists():
            ...
            st.warning(
                "Output file was not created. Check the errors above."
            )
